...
class TesteFluxoCompleto:
    ...
    def verificar_saldo_voltz(self):
        """Verifica o saldo disponível na carteira Voltz"""
        try:
            print("\n💰 Verificando saldo da carteira Voltz...")
            
            # Chama o endpoint de wallet para verificar saldo
            response = requests.get(f"{self.backend_url}/voltz/voltz_rest.php?action=get_wallet", timeout=15)
            print(f"[DEBUG] Resposta Voltz get_wallet: HTTP {response.status_code} - {response.text}")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get('success'):
                        ...
                        print(f"❌ Erro ao verificar saldo: {data.get('error', 'Erro desconhecido')}")
    ...
    def enviar_btc_lightning_real(self):
        ...
        try:
            ...
            taxa_estimada = max(1000, valor_sats * 0.01)  # Taxa estimada de 1% ou mínimo 1000 sats
            ...
            print(f"   💸 Taxa estimada: {taxa_estimada:,} sats")
            ...
            # 4. Chamar backend Voltz para pagar a invoice
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get('success'):
                        ...
                        print(f"❌ ERRO VOLTZ: Não foi possível enviar BTC")
                        print(f"🔍 Detalhes do erro: {error_msg}")
                        
                        # Análise específica de tipos de erro
                        if 'insufficient' in error_msg.lower() or 'saldo' in error_msg.lower():
                            print(f"💡 Diagnóstico: SALDO INSUFICIENTE na carteira Voltz")
                        elif 'network' in error_msg.lower() or 'timeout' in error_msg.lower():
                            print(f"💡 Diagnóstico: ERRO DE REDE ou TIMEOUT")
                        elif 'invalid' in error_msg.lower() or 'malformed' in error_msg.lower():
                            print(f"💡 Diagnóstico: INVOICE INVÁLIDA ou MALFORMADA")
                        elif 'route' in error_msg.lower() or 'path' in error_msg.lower():
                            print(f"💡 Diagnóstico: ERRO DE ROTEAMENTO Lightning")
                        else:
                            print(f"💡 Diagnóstico: ERRO GENÉRICO - verificar logs do servidor")
                        
                        return False
                except json.JSONDecodeError:
                    print(f"❌ ERRO: Resposta inválida do servidor Voltz")
                    print(f"🔍 Resposta bruta: {response.text}")
                    return False
            else:
                print(f"❌ ERRO HTTP: Não foi possível comunicar com Voltz")
                print(f"🔍 Status: {response.status_code}")
                print(f"🔍 Resposta: {response.text}")
                
                # Análise específica de códigos HTTP
                if response.status_code == 400:
                    print(f"💡 Diagnóstico: REQUISIÇÃO INVÁLIDA - verifique parâmetros")
                elif response.status_code == 401:
                    print(f"💡 Diagnóstico: NÃO AUTORIZADO - verifique credenciais")
                elif response.status_code == 403:
                    print(f"💡 Diagnóstico: ACESSO NEGADO - verifique permissões")
                elif response.status_code == 404:
                    print(f"💡 Diagnóstico: ENDPOINT NÃO ENCONTRADO")
                elif response.status_code == 500:
                    print(f"💡 Diagnóstico: ERRO INTERNO DO SERVIDOR")
                elif response.status_code == 502:
                    print(f"💡 Diagnóstico: BAD GATEWAY - servidor indisponível")
                elif response.status_code == 503:
                    print(f"💡 Diagnóstico: SERVIÇO TEMPORARIAMENTE INDISPONÍVEL")
                elif response.status_code == 504:
                    print(f"💡 Diagnóstico: GATEWAY TIMEOUT - servidor demorou para responder")
                else:
                    print(f"💡 Diagnóstico: ERRO HTTP GENÉRICO")
