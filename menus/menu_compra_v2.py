...
class MenuCompraV2:
    ...
    def validar_valor(self, valor_str: str) -> tuple[bool, float, str]:
        """Valida e converte valor informado"""
        try:
    ...
    def menu_redes(self):
        """Menu de seleção de redes (apenas Lightning por simplicidade)"""
        return [
            ["⚡ Lightning"],
            ["🔙 Voltar"]
        ]
    ...
    async def informar_valor(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # Salva valor e mostra resumo
        context.user_data['valor_brl'] = valor
        
        # Calcula valores (simulação simples)
        taxa_percentual = 5.0  # 5%
        comissao = valor * (taxa_percentual / 100)
        valor_liquido = valor - comissao
        
        # Cotação simulada (em produção usar API real)
        cotacao_btc = 350000.0  # R$ 350k
        valor_btc = valor_liquido / cotacao_btc
        valor_sats = int(valor_btc * 100_000_000)
        
        context.user_data['comissao'] = comissao
        context.user_data['valor_liquido'] = valor_liquido
        ...
        mensagem = (
            ...
            f"💎 Moeda: {context.user_data['moeda']}\n"
            f"⚡ Rede: {context.user_data['rede']}\n"
            f"💰 Valor: {self.formatar_brl(valor)}\n"
            f"💸 Comissão ({taxa_percentual}%): {self.formatar_brl(comissao)}\n"
            f"✅ Valor líquido: {self.formatar_brl(valor_liquido)}\n"
            f"₿ Você receberá: {valor_sats:,} sats\n"
            f"━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Confirma os dados?"
        )
        ...
        capture_step(chatid, "RESUMO_EXIBIDO", {
            "valor": valor,
            "comissao": comissao,
    ...
    async def _processar_pix(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            # Para Lightning, não precisa de endereço agora
            endereco_temp = "LIGHTNING_PENDING"
            
            if not DEPIX_ENABLED:
                # Modo simulação se API não disponível
            ...
            cobranca = pix_api.criar_pagamento(
                ...
                taxa=5.0,  # 5%
                forma_pagamento="PIX",
                send=context.user_data['valor_btc'],
                user_id=chatid,  # Usando chatid de forma consistente
            ...
            # Inicia verificação inteligente em background
            if txid:
                try:
                    # Agendar verificação em intervalos inteligentes: 30s, 2min, 5min
                    ...
                    # Fallback: verificação imediata em 30s
            ...
            mensagem = (
                ...
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"💰 Valor: {self.formatar_brl(valor_brl)}\n"
                f"⚡ Receberá: {context.user_data['valor_sats']:,} sats\n"
                f"🆔 ID: `{txid}`\n\n"
            ...
            capture_step(chatid, "PIX_CRIADO_SUCESSO", {
                "txid": txid,
                "valor": valor_brl,
                "valor_sats": context.user_data['valor_sats']
            })
